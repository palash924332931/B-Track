...
@Component({
  selector: 'app-dailypayment-report',
  templateUrl: './dailypayment-report.component.html',
  styleUrls: ['./dailypayment-report.component.scss'],
...
export class DailypaymentReportComponent implements OnInit {
  ...
  public carPaymentList: DailyCarLogReport[];
  ...
  fnGetDailyPayment() {
    ...
    this.reportService.fnGetDailyCarLogReport(this.UserId, LastUpdate).subscribe(
      (data: DailyCarLogReport[]) => {
        console.log(data);
        this.carPaymentList = data || [];
  ...
  fnSearchReport() {
    this.alertService.fnLoading(true);
    this.reportService.fnGetDailyCarLogReport(this.UserId, this.reportDate).subscribe(
      (data: any[]) => {
        this.carTypeTableBind.tableName = this.LT == 'bn' ? this.reportDate + ' তারিখে গাড়ির লেনদেনের প্রতিবেদন' : 'Payment Status of Date ' + this.reportDate,
          console.log(data);
        this.carPaymentList = data || [];
        this.alertService.fnLoading(false);
      },
  ...
  public carTypeTableBind = {
    ...
    tableName: this.LT == 'bn' ? this.configService.getCurrentDate() + ' তারিখে গাড়ির লেনদেনের প্রতিবেদন' : 'Payment Status of Date ' + this.configService.getCurrentDate(),
    ...
    tableColDef: [
      ...
      { headerName: this.LT == 'bn' ? 'চলমান গাড়ির সংখ্যা' : 'No. of on Route Bus', width: '10%', internalName: 'NoOfOnRootCar', sort: true, type: "button", onClick: "Yes", showTotal: true },
      ...
      { headerName: this.LT == 'bn' ? 'রুটে আয়' : 'Route Income', width: '8%', internalName: 'ReceivedOnRouteIncome', sort: true, type: "button", onClick: "Yes", showTotal: true },
      { headerName: this.LT == 'bn' ? 'ভিন্ন রুটে আয়' : 'Diff. Route Income', width: '10%', internalName: 'ReceivedDifferentRouteIncome', sort: true, type: "button", onClick: "Yes", showTotal: true },
      ...
      { headerName: this.LT == 'bn' ? 'পরিশোধকৃত টাকা' : 'Paid Amount (TK)', width: '10%', internalName: 'ReceivedAmount', sort: true, type: "button", onClick: "Yes", showTotal: true },
      { headerName: this.LT == 'bn' ? 'অপরিশোধকৃত গাড়ীর সংখ্যা' : 'No. of Due Bus', width: '20%', internalName: 'NoOfDueCar', sort: true, type: "button", onClick: "Yes", showTotal: true },
    ],
    enabledSearch: true,
    pageSize: 25,
  ...
  fnPtableCellClick(event: any) {
    ...
    if (event.cellName == 'NoOfCar') {
      ...
      this.carService.fnGetDailyPaymentReportSection(this.UserId, this.reportDate, record.CarTypeId, 'Paid Bus').then((data: any) => {
        console.log("data", data);
        this.alertService.fnLoading(false)
        this.configureableModalData = data || [];
    ...
    } else if (event.cellName == 'NoOfDueCar') {
      this.carService.fnGetDailyPaymentReportSection(this.UserId, this.reportDate, record.CarTypeId, 'Due Bus').then((data: any) => {
        console.log("data", data);
        this.alertService.fnLoading(false)
        this.configureableModalData = data || [];
      ...
      this.carService.fnGetDailyPaymentReportSection(this.UserId, this.reportDate, record.CarTypeId, 'Received On Route Income').then((data: any) => {
        console.log("data", data);
        this.alertService.fnLoading(false)
        this.configureableModalData = data || [];
      ...
      this.carService.fnGetDailyPaymentReportSection(this.UserId, this.reportDate, record.CarTypeId, 'Received Different Route Income').then((data: any) => {
        console.log("data", data);
        this.alertService.fnLoading(false)
        this.configureableModalData = data || [];
